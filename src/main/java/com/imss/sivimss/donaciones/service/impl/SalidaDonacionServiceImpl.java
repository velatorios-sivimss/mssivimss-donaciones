...
public class SalidaDonacionServiceImpl implements SalidaDonacionService {
	...
	private static final String REGISTRADO_CORRECTAMENTE = "118"; // Se ha registrado correctamente el registro de
																	// salida de ataúdes de donación.
	...
	public Response<Object> detalleContratanteRfc(DatosRequest request, Authentication authentication)
		...
		try {
			logUtil.crearArchivoLog(Level.INFO.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), " detalle contratante rfc ", CONSULTA, authentication);

			if (donacionRequest.getRfc() == null) {
				throw new BadRequestException(HttpStatus.BAD_REQUEST, INFORMACION_INCOMPLETA);
			}

			Response<Object> response = providerRestTemplate.consumirServicioObject(
					new SalidaDonacion().detalleContratanteRfc(request, donacionRequest).getDatos(),
			...
			if (response.getCodigo() == 200 && response.getDatos().toString().contains(NOM_PERSONA)) {
				...
				response = providerRestTemplate
						.consumirServicioObject(urlRfc.concat("/").concat(donacionRequest.getRfc()), 0);
				if (response.getCodigo() == 200
						&& response.getDatos().toString().toLowerCase().contains("ACTIVO".toLowerCase())) {
			...
			String consulta = new SalidaDonacion().detalleContratanteRfc(request, donacionRequest).getDatos()
			...
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), FALLO_AL_EJECUTAR_EL_QUERY + decoded, CONSULTA,
					authentication);
	...
	public Response<Object> detalleContratanteCurp(DatosRequest request, Authentication authentication)
		...
		try {
			...
			Response<Object> response = providerRestTemplate.consumirServicioObject(
					new SalidaDonacion().detalleContratanteCurp(request, donacionRequest).getDatos(),
					urlModCatalogos.concat("/consulta"), authentication);
			...
			if (response.getCodigo() == 200 && response.getDatos().toString().contains(NOM_PERSONA)) {
				...
				response = providerRestTemplate
						.consumirServicioObject(urlCurp.concat("/").concat(donacionRequest.getCurp()), 1);
				if (response.getCodigo() == 200 && !response.getDatos().toString().toLowerCase()
						.contains("LA CURP NO SE ENCUENTRA EN LA BASE DE DATOS".toLowerCase())) {
					response.setMensaje("externo");
					return response;
				} else {
			...
			String consulta = new SalidaDonacion().detalleContratanteCurp(request, donacionRequest).getDatos()
					.get(AppConstantes.QUERY).toString();
			String decoded = new String(DatatypeConverter.parseBase64Binary(consulta));
			log.error(ERROR_AL_EJECUTAR_EL_QUERY + decoded);
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
	...
	@Override
	public Response<Object> detalleSalidaAtaudDonado(DatosRequest request, Authentication authentication)
			throws IOException {
		UsuarioDto usuarioDto = new Gson().fromJson((String) authentication.getPrincipal(), UsuarioDto.class);
		try {
			logUtil.crearArchivoLog(Level.INFO.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), " detalle salida ataud donado ", CONSULTA, authentication);
			return MensajeResponseUtil.mensajeConsultaResponse(
					providerRestTemplate.consumirServicio(
							new SalidaDonacion().detalleSalidaAtaudDonado(request, usuarioDto).getDatos(),
							urlModCatalogos.concat("/consulta"), authentication),
					SIN_INFORMACION);
		} catch (Exception e) {
			String consulta = new SalidaDonacion().detalleSalidaAtaudDonado(request, usuarioDto).getDatos()
					.get(AppConstantes.QUERY).toString();
			String decoded = new String(DatatypeConverter.parseBase64Binary(consulta));
			log.error(ERROR_AL_EJECUTAR_EL_QUERY + decoded);
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
	...
	@Override
	public Response<Object> cantidadSalidaAtaudDonado(DatosRequest request, Authentication authentication)
			throws IOException {
		AgregarArticuloRequest agregarArticuloRequest = new Gson()
				.fromJson(String.valueOf(request.getDatos().get(AppConstantes.DATOS)), AgregarArticuloRequest.class);
		try {
			logUtil.crearArchivoLog(Level.INFO.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), " count salida ataud donado ", CONSULTA, authentication);

			if (agregarArticuloRequest.getModeloArticulo() == null) {
				throw new BadRequestException(HttpStatus.BAD_REQUEST, INFORMACION_INCOMPLETA);
			}

			Response<Object> respuestaGenerado = providerRestTemplate.consumirServicio(
					new SalidaDonacion().countSalidaAtaudDonado(request, agregarArticuloRequest).getDatos(),
					urlModCatalogos.concat("/consulta"), authentication);
			...
			String consulta = new SalidaDonacion().countSalidaAtaudDonado(request, agregarArticuloRequest).getDatos()
					.get(AppConstantes.QUERY).toString();
			String decoded = new String(DatatypeConverter.parseBase64Binary(consulta));
			log.error(ERROR_AL_EJECUTAR_EL_QUERY + decoded);
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
	...
	@Override
	public Response<Object> insertSalidaAtaudDonado(DatosRequest request, Authentication authentication)
			throws IOException {
		String consulta = "";
		...
		try {
			logUtil.crearArchivoLog(Level.INFO.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), " insert salida ataud donado ", ALTA, authentication);
			map.put("curp", donacionRequest.getCurp());
			map.put("rfc", donacionRequest.getRfc());
			DatosRequest datos = new DatosRequest();
			datos.setDatos(map);
			
			response = providerRestTemplate.consumirServicioObject(datos.getDatos(),
					urlModCatalogos.concat("/consultas/consultaPersona"), authentication);
			...
			if (response.getCodigo() == 200 && !response.getDatos().toString().contains("[]")) {
				...
				response = providerRestTemplate.consumirServicio(
						new SalidaDonacion().insertSalidaDonacionPrincipal(donacionRequest, usuarioDto),
						urlModCatalogos.concat("/insercion/salida/donacion"), authentication);
					 
				consulta = new SalidaDonacion().insertSalidaDonacionPrincipal(donacionRequest, usuarioDto).toString();
				if (200 == response.getCodigo()) {
				 
					response = providerRestTemplate.consumirServicio(
							new SalidaDonacion().actualizarSalidaDonacionPrincipal(donacionRequest, usuarioDto),
							urlModCatalogos.concat("/actualizar/multiples"), authentication);
							 
				}
			} else {
				 
				response = providerRestTemplate.consumirServicio(
						new SalidaDonacion().insertPersona(donacionRequest, usuarioDto),
						urlModCatalogos.concat("/insercion/salida/donacion"), authentication);
			...
			log.error(ERROR_AL_EJECUTAR_EL_QUERY + decoded);
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), FALLO_AL_EJECUTAR_EL_QUERY + decoded, ALTA,
					authentication);
	...
	public Response<Object> generarDocumentoControlSalidaDonacion(DatosRequest request, Authentication authentication)
		...
		try {
			logUtil.crearArchivoLog(Level.INFO.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), " generar documento control salida donacion ", CONSULTA,
					authentication);
			Map<String, Object> envioDatos = new SalidaDonacion().generarPlantillaControlSalidaDonacionPDF(
					new Gson().fromJson(String.valueOf(request.getDatos().get(AppConstantes.DATOS)),
							PlantillaControlSalidaDonacionRequest.class),
					nombrePdfControlSalida);
			return MensajeResponseUtil.mensajeConsultaResponse(
					providerRestTemplate.consumirServicioReportes(envioDatos, urlReportes, authentication),
					ERROR_AL_DESCARGAR_DOCUMENTO);
		} catch (Exception e) {
			log.error("Error.. {}", e.getMessage());
			logUtil.crearArchivoLog(Level.SEVERE.toString(), this.getClass().getSimpleName(),
					this.getClass().getPackage().toString(), "Fallo al ejecutar la plantilla : " + e.getMessage(),
